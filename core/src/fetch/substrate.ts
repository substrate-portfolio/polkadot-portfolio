...
export async function fetch_system(
  api: ApiPromise,
  account: string,
  chain: string
): Promise<Asset[]> {
...
export async function fetch_crowdloan(
  api: ApiPromise,
  account: string,
  chain: string
): Promise<Asset[]> {
...
export async function fetch_assets(
  api: ApiPromise,
  account: string,
  chain: string
): Promise<Asset[]> {
  ...
  const fetchAssetsPromise = allAssetIds.map(async (assetId) => {
    const assetAccount: Option<PalletAssetsAssetAccount> = await api.query.assets.account(
      assetId,
      account
    );
