...
async function main() {
	for (const uri in SUBSTRATE_BASED_CHAINS) {
		...
		const priceCent = await priceOf(overwrite_currency_name ? overwrite_currency_name : chain.toLowerCase());
		console.log(`✅ Connected to node: ${uri}  ${chain} [ss58: ${api.registry.chainSS58}][current price :${priceCent} cents]`);
