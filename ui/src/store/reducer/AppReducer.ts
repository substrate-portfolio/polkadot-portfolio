...
const removeAccount = (state: StoreState, accountId: string): StoreState => {
	...
	const updatedAccounts = accounts.includes(accountId)
...
const removeRegistry = (state: StoreState, network: string): StoreState => {
	...
	const updatedNetworks = networks.includes(network)
		? networks.filter((item) => item !== network)
		: networks;
	return {
		...state,
		visibility: {
...
const changeVisibility = (
	...
	let updatedNetworks: string[] = networks,
		updatedAccounts: string[] = accounts;
	if (network) {
		const hasNetwork = networks.includes(network);
		updatedNetworks = hasNetwork
			? networks.filter((item) => item !== network)
			: [...networks, network];
	}
	if (account) {
		const hasAccount = accounts.includes(account);
		updatedAccounts = hasAccount
